...
def hotel(request):
    return render(request, 'home/hotel.html')
def ngos(request):
    return render(request, 'home/ngos.html')
def gallery(request):
    return render(request, 'home/gallery.html')
def transport(request):
    return render(request, 'home/transport.html')
def tourist(request):
    return render(request, 'home/tourist.html')
def state(request):
...
def contact(request):
    if request.method=="POST":
        ...
        if len(name)<4 or len(email)<5 or len(phone)<10 or len(content)<10:
            messages.error(request, "Error, Please from fill properly..")
        else:
...
def about(request):
    ...
    # render use when we use templates
    #show for string than we use http : return HttpResponse('home/about.html')
...
def homepage(request):
    ...
    if request.method =="POST":
        try:
            ...
            #if len(room) == 0:
               # messages.warning(request,"No Passes Are Available on this time period")
...
def user_sign_up(request):
    if request.method =="POST":
        ...
        new_user.save()
        messages.success(request,"Registration Successfull, Enjoy *:*")
        return redirect("userloginpage")
...
def staff_sign_up(request):
    if request.method =="POST":
        ...
        try:
            if User.objects.all().get(username=user_name):
                messages.success(request,"User already exist, please re_register :)")
                return redirect("staffloginpage")
        except:
            pass
        ...
        new_user.save()
        messages.success(request,"Congratualtions, Organiser Registration Successfull, Create New Events *:*")
        return redirect("staffloginpage")
    else:
...
def add_new_room(request):
    ...
    if request.method == "POST":
        ...
        new_room.size       = str(request.POST['size'])
        new_room.capacity   = int(request.POST['capacity'])
        ...
        new_room.save()
        messages.success(request,"New Event/Trip Added Successfully")
    ...
    if request.method =="POST":
        ...
        for each_reservation in Reservation.objects.all().filter(room = room):
            if str(each_reservation.check_in) < str(request.POST['check_in']) and str(each_reservation.check_out) < str(request.POST['check_out']):
                ...
                messages.warning(request,"Sorry this Event is unavailable for Booking")
                return redirect("homepage")
        ...
        reservation.save()
        messages.success(request,"Congratulations! Travelishq User *:* ")
        return redirect("homepage")
    else:
        return HttpResponse('Access Denied')
